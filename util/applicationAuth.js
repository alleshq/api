...
module.exports = async (req, res, next) => {
    ...
    if (typeof authHeader === "string") {
        ...
        try {
            credentialsString = Buffer.from(authHeader.split(" ")[1], "base64").toString().split(":");
        } catch (err) {
            return res.status(401).json({err: "badAuthorization"});
        }
        if (credentialsString.length !== 2) return res.status(401).json({err: "badAuthorization"});

        applicationCredentials = {
            id: credentialsString[0],
            secret: credentialsString[1]
        };
    } else if (typeof req.body.client_id === "string" && typeof req.body.client_secret === "string") {
        applicationCredentials = {
